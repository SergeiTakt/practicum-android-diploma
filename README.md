...
Это Android-приложение для поиска работы через API вакансий. Пользователь может искать вакансии, применять фильтры (локация, отрасль, зарплата), просматривать подробную информацию по вакансии и сохранять интересные предложения в «Избранное» для офлайн-доступа.

## Использованные библиотеки и технологии

- **Jetpack Compose** (Material 3, Navigation Compose) — UI и навигация между экранами.
- **Koin** — внедрение зависимостей.
- **Retrofit** + **kotlinx.serialization** + **Gson** — работа с сетью и сериализацией данных.
- **Room** (+ KSP) — локальная база данных для сохранения избранных вакансий.
- **Coil** (включая SVG) — загрузка и отображение изображений.
- **Kotlin Coroutines** — асинхронные операции.
- **kotlinx.collections.immutable** — неизменяемые коллекции.
